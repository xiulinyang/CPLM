...
from transformers.models.gpt2.configuration_babygpt2 import GPT2Config
...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None, num_epochs=10, initial_slope=1.0, final_slope=0.1):
        ...
        self.update_alibi_slope()  # Set initial slope value
        ...
        self.reorder_and_upcast_attn = getattr(config, "reorder_and_upcast_attn", False)        



    def update_alibi_slope(self):
        """
        Update the ALiBi slope m using the formula m_t = m_0 * r^t.
        All attention heads will share the same slope m_t.
        """
        ...
        # Use the same slope for all heads
        self.register_buffer(
            "m",
            torch.tensor([m_t]).unsqueeze(-1).unsqueeze(-1)
        )
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None):
        ...
        alibi_bias = (self.m.to(query.device) * get_relative_positions(seq_len).to(query.device)).unsqueeze(0)
        attn_weights = nn.functional.softmax(attn_weights + alibi_bias, dim=-1)
