...
if __name__ == '__main__':
    ...
    checkpoints = sorted([x.name for x in num_checkpoints if 'main' not in x.name], key=lambda x: int(x.split('_')[-1]))
    ...
    if best_checkpoint:
        ...
        pd.DataFrame(dist).to_csv(f'{dataset}_results/distributions_{model_name_name}_best.csv')
    else:
        for checkpoint in checkpoints:
            ...
            pd.DataFrame(dist).to_csv(f'{dataset}_results/distributions_ckpt{checkpoint}.csv')
