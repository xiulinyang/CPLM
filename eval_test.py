...
def main():
    ...
    scorer = SimpleScorer(args.model_dir, device=args.device, tokenizer_dir=args.tokenizer_dir)
    acc, dist = eval_sent_pair(scorer, tokenizer, test)
    tag = Path(args.model_dir).name
    out_dir = f"{args.eval_dataset}_results"
    pd.DataFrame({"best": acc}).to_csv(f"{out_dir}/results_{tag}_epoch20.csv")
    pd.DataFrame(dist).to_csv(f"{out_dir}/distributions_{tag}_epoch20.csv")
    print("Done.")
